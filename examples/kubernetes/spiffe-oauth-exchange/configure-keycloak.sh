...
KCADMIN="kubectl exec -c keycloak $KC_POD -- /opt/keycloak/bin/kcadm.sh"
...
$KCADMIN create identity-provider/instances -r demo -s alias=spiffe -s providerId=spiffe -s config='{"trustDomain": "spiffe://example.org", "bundleEndpoint": "https://spire-spiffe-oidc-discovery-provider.spire.svc.cluster.local/keys"}'
...
$KCADMIN create clients -r demo -s clientId=alpha -s serviceAccountsEnabled=true -s standardFlowEnabled=true -s clientAuthenticatorType=federated-jwt -s attributes='{ "jwt.credential.issuer": "spiffe", "jwt.credential.sub": "spiffe://example.org/ns/default/sa/alpha", "standard.token.exchange.enabled": "true" }'
$KCADMIN create clients -r demo -s clientId=beta -s serviceAccountsEnabled=true -s standardFlowEnabled=true -s clientAuthenticatorType=federated-jwt -s attributes='{ "jwt.credential.issuer": "spiffe", "jwt.credential.sub": "spiffe://example.org/ns/default/sa/beta", "standard.token.exchange.enabled": "true" }'
$KCADMIN create clients -r demo -s clientId=gamma -s serviceAccountsEnabled=true -s standardFlowEnabled=true -s clientAuthenticatorType=federated-jwt -s attributes='{ "jwt.credential.issuer": "spiffe", "jwt.credential.sub": "spiffe://example.org/ns/default/sa/gamma", "standard.token.exchange.enabled": "true" }'
$KCADMIN create clients -r demo -s clientId=delta -s serviceAccountsEnabled=true -s standardFlowEnabled=true -s clientAuthenticatorType=federated-jwt -s attributes='{ "jwt.credential.issuer": "spiffe", "jwt.credential.sub": "spiffe://example.org/ns/default/sa/delta", "standard.token.exchange.enabled": "true" }'
